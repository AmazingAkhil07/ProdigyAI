...
export const PortfolioChecklist: React.FC<PortfolioChecklistProps> = ({ progress, onToggle }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-amber-800/30 rounded-xl p-5 card-hover animate-fadeInUp">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100 flex items-center">
          <i className="fas fa-briefcase mr-2 text-amber-500" />
          Portfolio Readiness
        </h2>
        <span className="text-xs font-semibold bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 px-2 py-1 rounded">
          ...
          >
            <div className="flex items-center space-x-3">
              <input 
                ...
                className="w-4 h-4 rounded text-amber-600 border-slate-300 dark:border-slate-600 focus:ring-amber-500 cursor-pointer"
              />
